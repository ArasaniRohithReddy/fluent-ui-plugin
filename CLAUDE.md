...
- **Figma:** call `fluent_figma_guidance` — and check entitlement *before* promising a workflow: a View/Collab seat or any Starter plan gets ~6 tool calls per **month**. Auth is the host's own OAuth flow; never ask for, store or forward a Figma token.

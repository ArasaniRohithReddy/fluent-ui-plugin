 ...
 */
async function stampReportVersionAtImport(reportDir) {
    try {
        const lib = await import(pathToFileURL(join(SCRIPTS_DIR, 'pbir', 'lib.mjs')).href);
        ...
        if (!ct)
            return { computed, previous: null, changed: false };
...
export function registerPbip(server) {
    server.registerTool('fluent_scaffold_pbip', {
        title: 'Scaffold a Fluent 2 Power BI PBIP/PBIR project',
        description: 'Create a Fluent 2-themed Power BI Project (PBIP) with the enhanced report format (PBIR) by copying the bundled, schema-validated template and renaming it. Writes files under outputDir. themeCollection.customTheme.reportVersionAtImport is COMPUTED from the scaffolded files (the same computation fluent_pbir_apply_theme uses), and the scaffolded visual carries no inline background/border/title overrides, so the result passes fluent_pbir_verify V1-V9 out of the box. Open the resulting .pbip in Power BI Desktop (enable preview: PBIP save option + PBIR enhanced report format).',
        inputSchema: {
            name: z
                .string()
                .min(1)
        ...
        const versionNote = 'error' in stamp
        ...
        return textResult(`Scaffolded Fluent 2 PBIP/PBIR project "${name}" into ${outputDir}\n\n` +

...
export function registerPbip(server) {
    server.registerTool('fluent_scaffold_pbip', {
        ...
        description: 'Create a Fluent 2-themed Power BI Project (PBIP) with the enhanced report format (PBIR) by copying the bundled, schema-validated template and renaming it. Writes files under outputDir. Open the resulting .pbip in Power BI Desktop (enable preview: PBIP save option + PBIR enhanced report format).',
        inputSchema: {
            name: z
                .string()
                .min(1)

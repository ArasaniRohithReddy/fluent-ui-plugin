...
export function registerTokens(server) {
    ...
    server.registerTool('fluent_get_token', {
        ...
        if (!name || !name.trim()) {
            return textResult('Empty token name. Pass a token name or fragment, e.g. fluent_get_token { name: "colorBrandBackground" }, ' +
        ...
        if (aliasPath) {
            ...
            if (hit)
                results[label(aliasPath)] = hit.value;
        }
 ...
 */
function globalSlotMap(t) {
    const ref = t?.color?.aliasReference;
    if (!ref)
        return undefined;
    const out = {};
    for (const [name, entry] of Object.entries(ref)) {
...
function aliasReferencesFor(t, resultKeys) {
    const ref = t?.color?.aliasReference;
    if (!ref)
        return {};
    const names = new Set();
...
function withShippedNames(typography) {
    if (!typography || typeof typography !== 'object' || !typography.fontFamilies)
        return typography;
    const families = typography.fontFamilies;
    const shipped = {};
    for (const [path, tokenName] of Object.entries(SHIPPED_NAME_BY_PATH)) {
        const leaf = path.split('.').pop();
        if (families[leaf] !== undefined)
            shipped[tokenName] = families[leaf];
    }
    return {
        ...typography,
        ...
        fontFamilyTokensNote: 'Use fontFamilyBase / fontFamilyMonospace / fontFamilyNumeric in code (tokens.fontFamilyBase, --fontFamilyBase). ' +

...
function walkTokens(obj, path, out) {
...
export function registerTokens(server) {
    server.registerTool('fluent_list_tokens', {
        ...
        description: 'List Fluent 2 design tokens by category (color, typography, spacing, borderRadius, strokeWidth, shadow, motion). For color, choose the theme (light/dark/highContrast). Values are concrete resolved values; each token X is exposed as the CSS variable --X and as tokens.X in Griffel makeStyles.',
        inputSchema: {
            category: z
        ...
        if (!t)
            return textResult('Token data not found at mcp/data/fluent-tokens.json.');
        const themeKey = theme === 'dark' ? 'semanticDark' : theme === 'highContrast' ? 'semanticHighContrast' : 'semanticLight';
        if (category === 'color') {
    ...
    server.registerTool('fluent_get_token', {
        ...
        description: 'Look up a Fluent 2 design token by name (e.g. colorBrandBackground, spacingHorizontalM, borderRadiusMedium, fontSizeBase300, shadow8, durationNormal). Returns concrete value(s) across light/dark/high-contrast where applicable plus the CSS variable name.',
        inputSchema: {
        ...
        const t = loadJson('fluent-tokens.json');
        if (!t)
            return textResult('Token data not found.');
        ...
        return textResult(`Matches for "${name}" — each token X maps to CSS variable --X and tokens.X in Griffel:\n\n` +

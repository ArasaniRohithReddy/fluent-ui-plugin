...
function flatten(data) {
    ...
    for (const cat of data.categories || []) {
        for (const v of cat.visuals || [])
            out.push({ ...v, category: cat.name });
    }
    for (const f of data.featurePages || [])
        out.push({ ...f, category: 'Feature (applies across visuals)' });
    return out;
}
 ...
 */
function chartMatchesQuery(c, q) {
    const hay = norm([c.name, c.category, c.role, c.description, c.whenToUse, c.powerbiEquivalent, c.dos.join(' '), c.donts.join(' '), c.npmPackage].join(' '));
    return norm(q).split(/\s+/).filter(Boolean).every((w) => hay.includes(w));
}
function render(items, limit, header) {
    ...
    shown.forEach((v, i) => {
        ...
        if (v.whenToUse)
            lines.push(`   When to use: ${v.whenToUse}`);
        if (v.fluent2)
            lines.push(`   Fluent 2: ${v.fluent2}`);
        if (v.stylePreset)
            lines.push(`   Style preset: ${v.stylePreset}`);
        if (v.subTypes && v.subTypes.length)
            lines.push(`   Sub-types: ${v.subTypes.join(' · ')}`);
        if (v.notes)
    ...
    if (items.length > limit)
        lines.push(`… ${items.length - limit} more. Narrow with category/showcasePage/query or raise "limit".`);
    return lines.join('\n');
}
const trim = (s, n) => (s.length > n ? s.slice(0, n - 1).trimEnd() + '…' : s);
function renderCharts(items, limit, header, palette) {
    const shown = items.slice(0, limit);
    const lines = [header, ''];
    shown.forEach((c, i) => {
        lines.push(`${i + 1}. **${c.name}** _(${c.maturity})_ — ${c.category}${c.role !== 'chart' ? ` · ${c.role}` : ''}`);
        if (c.description)
            lines.push(`   ${trim(c.description, 320)}`);
        if (c.whenToUse)
        ...
        lines.push(`   Theming: render inside <FluentProvider theme={webLightTheme}>; omit \`color\` on a series to cycle DataVizPalette, or pin one with DataVizPalette.color1…color${palette?.qualitativeCount ?? 40} / .success / .error.`);
        if (c.dos.length)
            lines.push(`   Do: ${trim(c.dos[0], 200)}`);
        if (c.donts.length)
            lines.push(`   Don't: ${trim(c.donts[0], 200)}`);
        if (c.a11y)
            lines.push(`   Accessibility: ${trim(c.a11y.replace(/\s*\n\s*-\s*/g, ' · ').replace(/^-\s*/, '').replace(/\s+/g, ' '), 260)}`);
        if (c.powerbiEquivalent)
            lines.push(`   Power BI equivalent: ${c.powerbiEquivalent}${c.powerbiNote ? ` — ${c.powerbiNote}` : ''}`);
        else
            lines.push('   Power BI equivalent: none (no first-party Power BI visual does this).');
...
export function registerPowerbiVisuals(server) {
    server.registerTool('fluent_powerbi_visuals', {
        ...
        const data = loadJson('powerbi-visuals.json');
        if (!data)
            return textResult('Power BI visual catalog not found at mcp/data/powerbi-visuals.json.');
        const chartsData = loadJson('fluent-charts.json');
        const wantPbi = surface !== 'fluent-charts';
        const wantCharts = surface !== 'powerbi' && !!chartsData;
        let items = flatten(data);
        if (category)
            items = items.filter((v) => norm(v.category).includes(norm(category)));
        if (showcasePage)
            items = items.filter((v) => norm(v.showcasePage || '').includes(norm(showcasePage)));
        if (query)
            items = items.filter((v) => matchesQuery(v, query));
        if (!wantPbi)
            items = [];
        let charts = wantCharts ? chartsData.charts : [];
        ...
        if (showcasePage)
            charts = [];
        if (category)
            charts = charts.filter((c) => norm(c.category).includes(norm(category)));
        if (query)
            charts = charts.filter((c) => chartMatchesQuery(c, query));
        const caveats = (chartsData?.meta?.unverified?.entries ?? []).map((u) => `${u.name} — ${u.note}`);
        if (!query && !category && !showcasePage) {
            ...
            if (wantPbi) {
                overview.powerbi = {
                    counts: data.counts,
                    categories: (data.categories || []).map((c) => ({ name: c.name, showcasePages: c.showcasePages, visuals: (c.visuals || []).map((v) => v.name) })),
                    featurePages: (data.featurePages || []).map((f) => f.name),
            ...
            if (wantCharts) {
                overview.fluentCharts = {
                    ...
                    packages: (chartsData.meta.packages || []).map((p) => ({
                        name: p.name,
                        version: p.version,
                        maturity: p.maturity,
                        use: p.use,
                    ...
                    byCategory: Object.fromEntries((chartsData.meta.categories || []).map((cat) => [cat, chartsData.charts.filter((c) => c.category === cat).map((c) => c.name)])),
                    dataVizPalette: {
                    ...
                    accessibility: chartsData.accessibility.rules.map((r) => r.rule),
        ...
        if (items.length === 0 && charts.length === 0) {
            return textResult(`No chart matched (surface=${surface}, category=${category ?? '-'}, showcasePage=${showcasePage ?? '-'}, query=${query ?? '-'}). Call with no arguments to list the categories on both surfaces.`);
        }
        const filterDesc = [category && `category="${category}"`, showcasePage && `showcasePage="${showcasePage}"`, query && `query="${query}"`].filter(Boolean).join(', ');
        const blocks = [];
        if (items.length)
            blocks.push(render(items, limit, `Power BI visuals — ${items.length} result(s)${filterDesc ? ' for ' + filterDesc : ''}:`));
        if (charts.length) {
            blocks.push(renderCharts(charts, limit, `Fluent 2 React charts (${chartsData.meta.reactPackage}@${chartsData.meta.reactPackageVersion}) — ${charts.length} result(s)${filterDesc ? ' for ' + filterDesc : ''}:`, chartsData.dataVizPalette));
            blocks.push(`Same colours on both surfaces: fluent_generate_powerbi_theme emits dataColors from DataVizPalette's ${chartsData.dataVizPalette.qualitativeCount} qualitative slots in slot order, so series N in the report and series N in the React chart are the same hex.`);
        }

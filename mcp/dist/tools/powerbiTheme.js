 ...
 */
function dataVizSeriesColors(mode, count) {
    const charts = loadJson('fluent-charts.json');
    const qualitative = charts?.dataVizPalette?.qualitative;
...
function semanticColor(mode, key) {
    const charts = loadJson('fluent-charts.json');
    const entry = (charts?.dataVizPalette?.semantic ?? []).find((s) => s.key === key);
    if (!entry)
        return null;
...
export function registerPowerbiTheme(server) {
    server.registerTool('fluent_generate_powerbi_theme', {
        ...
        if (name)
            theme.name = name;
        const mode = paletteTheme === 'dark' ? 'dark' : 'light';
        const series = dataVizSeriesColors(mode, dataColorCount);
        if (series)
            theme.dataColors = series.colors;

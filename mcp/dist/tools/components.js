...
export function registerComponents(server) {
    server.registerTool('fluent_search_components', {
        ...
        description: 'Search the Fluent 2 (React v9 / Web Components) catalog by name, category, or keyword. Returns matching components with category, description, React import, and web-component tag. Includes AI/Copilot components.',
        inputSchema: {
            query: z
                .string()
        ...
        if (!list.length)
            return textResult(`No components matching "${query}". Try a broader keyword.`);
        return textResult(`${list.length} match(es):\n\n` + JSON.stringify(list, null, 2));
    });
    server.registerTool('fluent_get_component', {
        title: 'Get a Fluent 2 component',
        description: "Get full details for a Fluent 2 component: real props (types/defaults), React import, web-component tag, accessibility notes, a code sample, plus usage guidance (when to use, anatomy, states, behavior, do/don't) and Storybook link.",
        inputSchema: {

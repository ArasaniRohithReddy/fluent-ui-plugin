...
function knownTags() {
    if (!tagCache) {
        const data = loadJson('web-component-tags.json');
 ...
 */
function wcDocument(title, body, notes = []) {
...
function webComponentsCopilotChat() {
...
function webComponents(kind) {
    ...
    if (unknown.length) {
        ...
        return (`<!-- GENERATOR BUG: these tags are not defined by @fluentui/web-components v3: ${unknown.join(', ')}.\n` +
...
export function registerCode(server) {
    server.registerTool('fluent_generate_code', {
        ...
        description: 'Scaffold Fluent 2 web code (React v9 or Web Components): an app shell with FluentProvider, a themed form, a card, or a Copilot chat starter. Both frameworks honour "kind". Uses makeStyles + tokens (React) or the token CSS custom properties setTheme() writes (Web Components) — no hardcoded values. Every <fluent-*> tag emitted is validated against the tag list @fluentui/web-components v3 actually defines.',
        inputSchema: {
            kind: z

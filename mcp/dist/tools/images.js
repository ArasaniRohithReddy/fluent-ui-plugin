...
function render(items, limit, header) {
    ...
    shown.forEach((m, i) => {
        ...
        if (m.docUrl)
            lines.push(`   Source page: ${m.docUrl}`);
        lines.push('');
    });
...
export function registerImages(server) {
    server.registerTool('fluent_get_images', {
        ...
        const data = loadJson('fluent-images.json');
        ...
        if (owner)
            items = items.filter((m) => matchesOwner(m, owner));
        if (kind && norm(kind) !== 'all')
            items = items.filter((m) => norm(m.kind) === norm(kind));
        if (type && type !== 'all')
        ...
        if (!owner && !kind && !query && (!type || type === 'all') && (!verdict || verdict === 'any')) {
            ...
            for (const m of data.media)
                owners[m.owner] = (owners[m.owner] || 0) + 1;
            const overview = {
        ...
        if (items.length === 0) {
            return textResult(`No Fluent 2 media matched (owner=${owner ?? '-'}, kind=${kind ?? '-'}, type=${type}, verdict=${verdict}, query=${query ?? '-'}). Try a broader filter, or call with no arguments to list available owners.`);
        }

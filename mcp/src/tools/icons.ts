...
interface Family {
  ...
  metaphor?: string[];
  rtl?: string;
  /** Ships exports but has no upstream design record: no description, no metaphors. */
  noDesignRecord?: boolean;
}
interface IconData {
  meta: {
    validSizes: number[];
...
const load = (): IconData | null => loadJson<IconData>('fluent-icons.json');
...
const baseWords = (base: string) =>
  base.replace(/([a-z0-9])([A-Z])/g, '$1 $2').replace(/([A-Z]+)([A-Z][a-z])/g, '$1 $2').toLowerCase().split(/\s+/).filter(Boolean);
...
function pickVariant(
  ...
  wantSize?: number,
  wantStyle?: string,
): { style: string; size: number | null } | null {
  ...
  for (const style of styleOrder) {
    ...
    if (wantSize) {
      if (sizes?.includes(wantSize)) return { style, size: wantSize };
      continue;
    }
    if (sizes?.length) {
      for (const s of DEFAULT_SIZE_ORDER) if (sizes.includes(s)) return { style, size: s };
      return { style, size: sizes[0] };
    }
    if (f.resizable?.includes(style)) return { style, size: null };
  }
  // Requested combination is unavailable — fall back so the caller still gets a
  // real name rather than nothing.
  if (wantSize || wantStyle) return pickVariant(f, undefined, undefined);
  return null;
}

const exportName = (base: string, size: number | null, style: string) => `${base}${size ?? ''}${style}`;

function sizeSummary(f: Family): string {
  const parts: string[] = [];
  for (const [style, sizes] of Object.entries(f.variants || {})) {
    parts.push(`${style} ${sizes.join('/')}`);
  }
  if (f.resizable?.length) parts.push(`resizable (no size in the name): ${f.resizable.join(', ')}`);
  return parts.join(' · ') || 'none';
}

function score(f: Family, tokens: string[], qNorm: string, wantSize?: number, wantStyle?: string): Candidate | null {
  ...
  if (covered === 0) {
    for (const t of tokens) {
      let best = 0;
      let reason = '';
      if (nameSet.has(t)) { best = 300; reason = 'name word'; }
      else if ([...nameSet].some((w) => w.startsWith(t))) { best = 200; reason = 'name prefix'; }
      if (metaphors.includes(t) && 260 > best) { best = 260; reason = 'metaphor'; }
      else if (metaphorWords.has(t) && 230 > best) { best = 230; reason = 'metaphor word'; }
      else if (best < 150 && metaphors.some((m) => m.includes(t))) { best = 150; reason = 'metaphor'; }
      if (best < 90 && descW.has(t)) { best = 90; reason = 'description'; }
      if (best === 0) {
  ...
  if (wantSize) {
    ...
    total += has ? 80 : -400;
    if (!has) why.push(`no ${wantSize}px`);
  }
  if (wantStyle) {
    const has = allStyles.has(wantStyle);
    total += has ? 60 : -300;
    if (!has) why.push(`no ${wantStyle}`);
  }
  if (allStyles.has('Regular') && allStyles.has('Filled')) total += 15;
  // Colour-only families are a trap: the package README warns against Color.
  if (allStyles.size && !allStyles.has('Regular') && !allStyles.has('Filled')) total -= 150;
  // An icon with no upstream design record is real but undocumented, so prefer
  // a documented one when both match.
  if (f.noDesignRecord) total -= 40;
...
function renderHit(data: IconData, f: Family, i: number, wantSize?: number, wantStyle?: string): string {
  ...
  if (allStyles.has('Regular') && allStyles.has('Filled')) {
    ...
    const filled = f.variants?.Filled?.includes(size as number) || (size === null && f.resizable?.includes('Filled'))
      ? exportName(f.base, size, 'Filled')
      : null;
    const regular = f.variants?.Regular?.includes(size as number) || (size === null && f.resizable?.includes('Regular'))
  ...
  if (f.metaphor?.length) lines.push(`   Metaphors: ${f.metaphor.join(', ')}`);
  if (f.rtlBase) {
    ...
    lines.push(
      `   RTL: directionType "${f.rtl}" — ${f.rtl === 'mirror' ? 'this icon flips in right-to-left locales' : 'a separate right-to-left asset exists for this icon'}.`,
    );
  }
...
function guidanceFooter(g: IconData['guidance'], opts: { size?: number; hasColorOnly?: boolean } = {}): string {
  const lines = ['', '---', `Naming: ${g.naming}`, `Sizes: ${g.sizes}`];
  if (opts.size === 12) lines.push('12px warning: 12px icons are informational only — never put one in an interactive control.');
  lines.push(`Regular vs Filled: ${g.styles.Regular} ${g.styles.Filled}`);
  if (opts.hasColorOnly) lines.push(`Color: ${g.styles.Color}`);
...
const cleanCandidate = (q: string) => q.trim().replace(/^[<{[('"`\s]+/, '').replace(/[>}\])'"`;,.\s/]+$/, '');
const looksLikeExport = (q: string) => EXPORT_SHAPED.test(cleanCandidate(q));

function findFamilyByBase(data: IconData, base: string): Family | null {
  const n = norm(base);
 ...
 */
function validateName(data: IconData, raw: string, wantSize?: number, wantStyle?: string): { ok: boolean; text: string } | null {
...
function validateResolved(
  ...
  if ((size === null && resizable.includes(style)) || (size !== null && styleSizes.includes(size))) {
    ...
    if (size === null) lines.push('This is the resizable export: no size in the name, it scales with fontSize / height / width.');
    if (size === 12) lines.push('12px icons are informational only — never put one in an interactive control.');
    if (style === 'Color') lines.push('Color variants are discouraged: non-compliant with Windows High Contrast Mode, gradient-id collisions, and dark-theme contrast failures. Prefer Regular or Filled.');
    if (entry.rtlBase) {
    ...
    } else if (entry.rtl) lines.push(`RTL: directionType "${entry.rtl}".`);
  ...
  if (size !== null && pool.length) {
    ...
    for (const s of closest) alternatives.push(`${real}${s}${preferredStyle}`);
  } else if (pool.length) {
    for (const s of pool.slice(0, 3)) alternatives.push(`${real}${s}${preferredStyle}`);
  }
  if (resizable.includes(preferredStyle)) alternatives.push(`${real}${preferredStyle}`);
...
function nearestBases(data: IconData, base: string, limit: number): Array<{ base: string; d: number }> {
  const target = norm(base);
  const targetWords = new Set(baseWords(base));
  const scored: Array<{ base: string; d: number }> = [];
  for (const f of data.families) {
    ...
    if (baseWords(f.base).some((w) => targetWords.has(w))) d -= 4;
    if (n.startsWith(target) || target.startsWith(n)) d -= 3;
  ...
  server.registerTool(
    ...
    async ({ query, size, style, limit }) => {
      ...
      if (size !== undefined && !validSizes.includes(size)) {
        return textResult(
          `size=${size} is not a Fluent icon size. Fluent 2 system icons are drawn at ${validSizes.join(', ')} px only — there is no ${size}px icon, and no export name contains ${size}.\n\n` +
            `${data.guidance.sizes}\n\nRe-run with one of: ${validSizes.join(', ')}, or omit "size" to see everything a family ships.`,
        );
      }

      const q = query.trim();
      if (!q) return textResult('Provide a query — what the icon should depict, or an export name to validate.');
      ...
      const anyColorOnly = hits.some(
        (h) => !h.family.variants?.Regular && !h.family.variants?.Filled && !(h.family.resizable || []).some((s) => s === 'Regular' || s === 'Filled'),
      );
...
function searchFamilies(
  ...
  const variants: Array<{ tokens: string[]; weight: number }> = [{ tokens, weight: 1 }];
  tokens.forEach((t, i) => {
    for (const syn of INTENT_SYNONYMS[t] || []) {
      const swapped = [...tokens.slice(0, i), ...words(syn), ...tokens.slice(i + 1)];
      if (swapped.join(' ') !== tokens.join(' ')) variants.push({ tokens: swapped, weight: 0.86 });
    }
  });
  ...
  for (const v of variants.slice(0, 16)) {
    ...
    for (const f of data.families) {
      ...
      if (!prev || c.score > prev.score) best.set(f.base, c);
    }
  }

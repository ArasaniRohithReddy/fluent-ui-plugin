 ...
 */
function codeTokenBridgeFor(topic: string): Record<string, unknown> | undefined {
  const tokens = loadJson<any>('fluent-tokens.json');
  const entries: any[] = tokens?.designNameBridge?.entries ?? [];
  ...
  return {
    note:
      'The design names on this page are NOT the code token names. Every row below carries codeToken — the token ' +
    ...
    ...(traps.length
      ? {
          silentValueTraps: traps.map(
            (e) =>
              `"${e.designName}" = ${e.designValue} is tokens.${e.codeToken}. tokens.${e.expectedCodeName} exists and ` +
...
function bridgeCounts(): Record<string, number> {
  const tokens = loadJson<any>('fluent-tokens.json');
  const out: Record<string, number> = {};
  for (const e of (tokens?.designNameBridge?.entries ?? []) as any[]) {
    if (e?.topic) out[e.topic] = (out[e.topic] ?? 0) + 1;
  }
  return out;
}

...
interface MediaItem {
  ...
  url: string;
  labels?: string[];
  /** Literal copy rendered INSIDE the image, recovered by vision OCR. Distinct
   *  from alt, which describes the image. On Responsible AI and content pages
   *  the exact wording is the guidance, so agents need the strings themselves. */
  ocrText?: string;
  verdict?: string;
}
...
function inferVerdict(alt?: string): string | undefined {
...
function render(items: MediaItem[], limit: number, header: string): string {
  ...
  shown.forEach((m, i) => {
    ...
    if (m.ocrText) lines.push(`   On-screen text: ${m.ocrText}`);
...
export function registerImages(server: McpServer): void {
  server.registerTool(
    ...
    async ({ owner, kind, type, verdict, query, limit }) => {
      ...
      if (verdict && verdict !== 'any') items = items.filter((m) => (m.verdict || inferVerdict(m.alt)) === verdict);

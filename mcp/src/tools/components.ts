 ...
 */
function rejectEmpty(value: string, param: string, tool: string): string | null {
  if (value && value.trim()) return null;
  return (
    `\`${param}\` is required and was empty${value ? ' (whitespace only)' : ''}. ` +
 ...
 */
function v8Index(): Map<string, string> {
  const data = loadJson<any>('fluent-v8.json');
  ...
  for (const source of [
    ...
    for (const key of Object.keys(source || {})) {
      if (!index.has(key.toLowerCase())) index.set(key.toLowerCase(), key);
    }
  }
  return index;
}
...
function v8CrossReference(query: string, hasExactV9: boolean): Record<string, unknown> | null {
...
export function registerComponents(server: McpServer): void {
  server.registerTool(
    'fluent_search_components',
    {
      ...
      description:
        'Search the Fluent 2 (React v9 / Web Components) catalog by name, category, or keyword. Returns matching components with category, maturity, description, React import, and web-component tag. Includes AI/Copilot components and matches the design-site pattern names ("Chat input" finds ChatInput).',
      inputSchema: {
        query: z
          .string()
    ...
    async ({ query }) => {
      ...
      if (empty) return textResult(empty);
      ...
      // one merged record.
      const siteNames = (c: any) =>
        [c.siteGuidance?.name, c.slug, ...(c.apiComponents || [])].filter(Boolean).map(String);
      ...
      for (const c of comps) for (const n of siteNames(c)) known.add(norm(n));
      ...
      const rank = (c: any) => {
        ...
        if (name === q) return 0;
        if (siteNames(c).some((n) => n.toLowerCase() === q)) return 0;
        if (name.startsWith(q)) return 1;
        if (name.includes(q)) return 2;
        return 3;
      };
      const maturityRank = (m: string) =>
        ['stable', 'preview', 'compat', 'utility', 'migration', 'deprecated'].indexOf(m || 'stable');
      matches.sort(
        (a, b) => rank(a) - rank(b) || maturityRank(a.maturity) - maturityRank(b.maturity) || a.name.localeCompare(b.name)
      );
      const LIMIT = 80;
      ...
      if (!list.length) {
        ...
        return textResult(
          `No components matching "${echo(query)}".` +
  ...
  server.registerTool(
    ...
    async ({ name }) => {
      ...
      const data = loadJson<any>('fluent-components.json');
      const usageAll = loadJson<any[]>('fluent-components-usage.json') || [];
      ...
      const bySite = comps.filter(
        (c) =>
          String(c.siteGuidance?.name || '').toLowerCase() === q ||
      ...
      let api: any[] = [];
      let matchedBy = '';
      if (byId.length) { api = byId; matchedBy = 'catalog id'; }
      else if (byName.length) { api = byName; matchedBy = 'exact export name'; }
      else if (bySite.length) { api = bySite; matchedBy = `design-site pattern name "${bySite[0].siteGuidance?.name || echo(name)}"`; }
      else if (byNorm.length) { api = byNorm; matchedBy = `normalised name match on "${byNorm[0].name}"`; }
      else if (partial.length) { api = partial; matchedBy = `partial name match on "${partial[0].name}" — not an exact match for "${echo(name)}"`; }
      ...
      const usage =
        usageAll.find((u) => u.name.toLowerCase() === q) ||
        (primary &&
          usageAll.find(
            (u) =>
              (primary.slug && norm(u.slug || '') === norm(primary.slug)) ||
      ...
      if (!primary && !usage) {
        return textResult(
          `No Fluent 2 component "${echo(name)}".` +
            (v8

...
export function registerAccessibility(server: McpServer): void {
  server.registerTool(
    'fluent_accessibility_checklist',
    {
      ...
      description:
        'Return the Fluent 2 accessibility checklist (contrast, focus, keyboard, the v9 focus-management API (tabster hooks, focus indicators, AriaLiveAnnouncer), target size, names/roles, high contrast, reduced motion, per-surface notes) to self-review a UI against WCAG 2.1 AA.',
      inputSchema: {
        surface: z

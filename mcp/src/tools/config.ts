...
function readJsonSafe<T = any>(file: string): T | null {
  return readJsonState<T>(file).value;
}

/**
 * The full state of a JSON file on disk. `missing` and `invalid` MUST stay
 * distinguishable: collapsing them is what let a corrupt fluent.config.json be
 * reported as absent and then silently overwritten with a stub, destroying the
 * user's file.
 */
interface JsonState<T> {
  /** The file exists on disk with content (whether or not it parses). */
  exists: boolean;
  /** It exists AND parsed into a usable object. */
  parsed: boolean;
  value: T | null;
  /** Why it did not parse — surfaced to the caller verbatim. */
  error?: string;
}

function readJsonState<T = any>(file: string): JsonState<T> {
  let raw: string;
  try {
  ...
  if (!raw) return { exists: false, parsed: false, value: null };
  let parsedValue: any;
  try {
  ...
  return { exists: true, parsed: true, value: parsedValue as T };
}
...
// ---------------------------------------------------------------------------
// Local content overlay (provenance)
// ---------------------------------------------------------------------------
//
// mcp/data/local/ is gitignored. It holds guidance the published datasets
// withhold for licensing reasons (sign-in-gated Microsoft pages — see NOTICE),
// and the lookup tools merge it back at runtime when it is present. That is
// correct, but it means a checkout that HAS it answers differently from a fresh
// clone. Reporting it here makes "what would a clone see?" a one-call question
// instead of a hidden discrepancy.

interface OverlayFileReport {
  file: string;
  records: number;
  keys?: string[];
  error?: string;
...
function describeLocalOverlay(): Record<string, unknown> {
  const dir = join(DATA_DIR, 'local');
  const base = {
    dir,
    what:
      'Guidance withheld from the published repo for licensing reasons (sign-in-gated Microsoft pages; see NOTICE). ' +
  ...
  let names: string[] = [];
  try {
    if (!existsSync(dir)) return { present: false, ...base, files: [], totalRecords: 0, note: 'Not present — this checkout returns exactly what a fresh clone returns.' };
  ...
  const files: OverlayFileReport[] = [];
  let totalRecords = 0;
  for (const file of names) {
    const state = readJsonState<Record<string, unknown>>(join(dir, file));
  ...
  return {
    ...
    totalRecords,
    note:
      `This checkout restores ${totalRecords} record(s) from ${files.length} overlay file(s). Responses built from them are ` +
    ...
    agentInstruction:
      'When demoing or documenting output, state which records came from the overlay: a user of the published plugin ' +
...
const PRESET_ENUMS: Record<string, string[]> = {
...
type Validation = { ok: true; value: any } | { ok: false; error: string; hint?: string };

/** Known preset paths that look like the one the caller asked for. */
function nearestPaths(key: string, limit = 8): string[] {
  ...
  const scored = all
    .map((p) => {
      ...
      let score = 0;
      if (pl === lower) score = 100;
      else if (pl.includes(lower) || lower.includes(pl)) score = 60;
      else if (pl.endsWith('.' + leaf) || pl === leaf) score = 50;
      else if (pl.includes(leaf)) score = 30;
      else if (leaf.length > 3 && pl.split('.').some((seg) => seg.startsWith(leaf.slice(0, 4)))) score = 10;
      return { p, score };
    })
    .filter((s) => s.score > 0)
...
function validatePreset(key: string, rawValue: any): Validation {
  ...
  if (typeof value === 'number') {
    if (!Number.isFinite(value)) return { ok: false, error: `"${key}" must be a finite number.` };
...
export function registerConfig(server: McpServer): void {
        ...
        JSON.stringify(
          {
            ...
            ...(broken
              ? {
                  agentInstruction:
                    'A file exists but could not be parsed, so its settings are NOT in the resolved config below. ' +
    ...
    'fluent_init_config',
    {
      ...
      inputSchema: {
        ...
        createDir: z
          ...
          .default(false)
          .describe(
            'Create projectDir even when its PARENT is missing too. Off by default: a typo used to materialize a whole directory tree at the drive root. One missing level under an existing parent is always created.'
          ),
      },
    },
    async ({
          ...
          JSON.stringify(
            {
              ...
              error:
                'fluent.config.json exists but could not be parsed. Refusing to overwrite it — the current contents would be lost.',
  ...
  server.registerTool(
    ...
    async ({ projectDir, key, value }) => {
      ...
      const root = projectRoot(projectDir);
      const cfgPath = configPathFor(root);
      const state = readJsonState<Record<string, any>>(cfgPath);
          ...
          JSON.stringify(
            {
              ...
              error:
                'fluent.config.json exists but could not be parsed. Refusing to write — doing so would discard the file\'s current contents.',
      ...
      const cfg: Record<string, any> = state.parsed && state.value ? state.value : { $schema: SCHEMA_URL };
  ...
  server.registerTool(
    ...
    async ({ projectDir, question, answer, id, scope, surface, source }) => {
          ...
          JSON.stringify(
            {
              ...
              error:
                '.fluent/memory.json exists but could not be parsed. Refusing to write — every recorded decision in it would be lost.',
      ...
      const existingIndex = mem.decisions.findIndex((d: any) => d && d.id === decisionId);
      ...
      let preferenceMirrored: string | null = null;
      let preferenceRejected: string | null = null;

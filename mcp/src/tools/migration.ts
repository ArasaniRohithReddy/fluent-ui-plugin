...
export function registerMigration(server: McpServer): void {
  server.registerTool(
    ...
    async ({ scenario, maxChars }) => {
      ...
      if (scenario === 'all') {
        const entries: Record<string, { chars: number; summary: string }> = {};
      ...
      if (!section) {
        return textResult(
          `No guidance for scenario "${scenario}". Valid scenarios: ${Object.keys(map).join(', ')}, all.`,
        );
      }
      return textResult(
        capped(
          ...
          maxChars,
          `Request a narrower scenario, or raise maxChars. Scenario "${scenario}" is ${sizeOf(section).toLocaleString('en-US')} characters in full.`,
        ),
      );
    }
  );

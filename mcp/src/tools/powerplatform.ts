...
export function registerPowerplatform(server: McpServer): void {
  server.registerTool(
    ...
    async ({ surface, maxChars }) => {
      ...
      if (surface === 'myths') {
        if (!data.correctedMyths) return textResult('No corrected myths recorded.');
        return textResult(
          capped(JSON.stringify(data.correctedMyths, null, 2), maxChars, 'Request a single surface instead.')
        );
      }
      if (surface === 'applies') {
        if (!data.fluentAppliesBySurface) return textResult('No per-surface applicability matrix recorded.');
        return textResult(
          capped(JSON.stringify(data.fluentAppliesBySurface, null, 2), maxChars, 'Request a single surface instead.')
        );
      }
      if (surface === 'all') {
        ...
        return textResult(
          buildIndex(entries, {
            ...
            extra: {
              checkFirst:
                'Before recommending anything, call surface:"applies" (does Fluent apply to that surface at all?) and surface:"myths" (corrected false claims).',
            },
          })
        );
      }
      ...
      return textResult(
        capped(
          ...
          maxChars,
          `Surface "${surface}" is larger than the cap; raise maxChars to read it whole.`
        )
      );
    }
  );

...
function loadPbir(file: string): Promise<any> {
  const key = file;
  if (!modules.has(key)) {
...
function assertInside(reportDir: string, target: string): void {
...
const reportDirArg = z
  ...
  .min(1)
  .describe(
    'Path to the PBIR report folder (the *.Report directory that contains definition/pages), or the PBIP project root that holds exactly one *.Report folder. A binary .pbix or a PBIR-Legacy report.json cannot be used.'
  );
...
export function registerPbir(server: McpServer): void {
  // -------------------------------------------------------------------------
  // 1) audit
  // -------------------------------------------------------------------------
  server.registerTool(
    'fluent_pbir_audit',
    {
      ...
      inputSchema: {
        ...
        themePath: z
          ...
          .optional()
          .describe(
            'Optional theme JSON to compute ownership against. Defaults to the theme already registered in the report, then to the built-in Fluent 2 declarations.'
          ),
        top: z
          .number()
          .int()
    ...
    async ({ reportDir, themePath, top, format }) => {
      try {
        ...
        return textResult(
          format === 'json' ? JSON.stringify(result, null, 2) : mod.formatAudit(result)
        );
      } catch (err) {
  ...
  server.registerTool(
    ...
    async ({ reportDir, themeJson, themePath, themeName, dryRun, format }) => {
      try {
        ...
        return textResult(
          format === 'json' ? JSON.stringify(result, null, 2) : mod.formatApply(result)
        );
      } catch (err) {
        return textResult(errorText(err));
      }
  ...
  server.registerTool(
    'fluent_pbir_normalize_inline',
    {
      ...
      inputSchema: {
        ...
        policy: z
          ...
          .default('theme-wins')
          .describe(
            'theme-wins deletes the theme-owned inline override so the theme applies (the fix). report is a dry-run census that never writes. remap-colors rewrites inline hex colors to the nearest theme color and is an ANTI-PATTERN kept only for cases where an override must survive.'
          ),
        keys: z
          .array(z.string())
          .optional()
          .describe(
            'Container cards to normalize. Defaults to background, border, visualHeader, title. Keys the theme does not own are ignored and reported.'
          ),
        visualTypes: z
          .array(z.string())
          .optional()
        ...
        includeBookmarked: z
          ...
          .default(false)
          .describe(
            'Normalize visuals whose formatting a bookmark captured. Off by default because the bookmark snaps the old style back; re-capture those bookmarks in Power BI Desktop afterwards.'
          ),
        includeDataObjectTypography: z
          .boolean()
          .default(false)
          .describe(
            'Also delete fontFamily/fontSize from data-role cards under visual.objects so the theme textClasses apply. Off by default because visual.objects is data formatting.'
          ),
        themePath: z
          .string()
          .optional()
    ...
    async ({
      ...
      try {
        ...
        return textResult(
          format === 'json' ? JSON.stringify(result, null, 2) : mod.formatNormalize(result)
        );
      } catch (err) {
        return textResult(errorText(err));
      }
  ...
  server.registerTool(
    ...
    async ({
      ...
      try {
        ...
        return textResult(
          format === 'json' ? JSON.stringify(result, null, 2) : mod.formatVerify(result)
        );
      } catch (err) {
        return textResult(errorText(err));
      }

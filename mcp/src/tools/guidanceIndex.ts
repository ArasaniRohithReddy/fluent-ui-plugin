...
export interface IndexEntry {
  /** Extra facts to show per entry (counts, titles, flags). */
  [k: string]: unknown;
}

export interface IndexOptions {
  /** Tool call the caller should make next, with a real example key substituted in. */
  requestOne: string;
  /** What the index is an index OF. */
  what: string;
  /** Optional extra top-level fields (e.g. $meta). */
  extra?: Record<string, unknown>;
  /** Key the entry map is published under (defaults to "entries"). */
  entriesKey?: string;
}
...
export function buildIndex(
  ...
  const totalChars = Object.values(entries).reduce(
    (n, e) => n + (typeof e.chars === 'number' ? e.chars : 0),
    0
  );
  return JSON.stringify(
    {
      ...
      reason:
        `Returning everything would be about ${totalChars.toLocaleString('en-US')} characters, ` +
        'large enough to displace the rest of the conversation. This is the index instead.',
      requestOne: opts.requestOne,
      agentInstruction:
        'This is an INDEX, not content. Do not answer from it and do not infer any entry\'s content from its ' +
 ...
 */
export function capped(body: string, maxChars: number, hint: string): string {
  if (!Number.isFinite(maxChars) || maxChars <= 0 || body.length <= maxChars) return body;
  return (
    `TRUNCATED: this response is ${body.length.toLocaleString('en-US')} characters, above the ${maxChars.toLocaleString('en-US')}-character cap.\n` +
    `${hint}\n` +

...
export function registerPowerbiTheme(server: McpServer): void {
  server.registerTool(
    'fluent_generate_powerbi_theme',
    {
      ...
      description:
        'Generate a valid, Fluent 2-aligned Power BI report theme JSON (dataColors, foreground/background levels, status colors, Segoe UI textClasses, and visualStyles "visual defaults"). Optionally recolor to a brand hex. Import in Power BI Desktop via View > Themes > Browse for themes.',
      inputSchema: {
        brandColor: z
          .string()
          .regex(/^#?[0-9a-fA-F]{6}$/)

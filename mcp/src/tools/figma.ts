...
interface FigmaHost {
  id: string;
  name?: string;
  catalogConfirmed?: boolean | null;
  figmaDocumentsThisHost?: boolean;
  configKey?: string;
  urlField?: string;
  type?: string | null;
  configPath?: string;
...
interface FigmaData {
  ...
  rateLimitExemptTools?: string[];
  remoteOnlyTools?: unknown;
  entitlementSummary?: string;
  catalogGate?: Record<string, unknown>;
  hosts?: FigmaHost[];
  hostConfigNotes?: unknown;
  codeConnect?: Record<string, unknown>;
  fluentFigmaResources?: unknown;
  pluginCapability?: Record<string, unknown>;
  fluentFigmaPlugins?: { name?: string; official?: boolean; publisherNote?: string }[];
  fluentFigmaPluginsMeta?: Record<string, unknown>;
  fluentKitTiers?: unknown;
  fluentKitTiersImplication?: string;
  workflow?: unknown;
  prerequisiteMatrix?: unknown;
  unverified?: unknown;
}

const load = (): FigmaData | null => loadJson<FigmaData>('figma.json');
...
const DTCG_TYPES_USED = ['color', 'dimension', 'duration', 'number', 'fontFamily', 'fontWeight'] as const;
type DtcgType = (typeof DTCG_TYPES_USED)[number];
...
export interface DtcgBundle {
  ...
  totalTokens: number;
  typesUsed: string[];
  notExpressible: { category: string; tokens: number; reason: string }[];
}
 ...
 */
function cssColorToDtcgHex(value: string): string | null {
  const v = value.trim();
...
export function buildFluentDtcg(): DtcgBundle | null {
  const t = loadJson<any>('fluent-tokens.json');
  if (!t) return null;

  /** Which `tokens.*` names really exist, straight from the generated dataset — never guessed. */
  const realTokenNames: Record<string, string> = t.color?.cssVarByToken ?? {};

  const token = (type: DtcgType, value: string | number, description: string, codeName?: string): DtcgToken => {
    const tok: DtcgToken = { $type: type, $value: value, $description: description };
  ...
  const count = (g: DtcgGroup): number =>
    Object.values(g).reduce<number>((n, v) => n + ('$value' in (v as any) ? 1 : count(v as DtcgGroup)), 0);

  // ---- Global collection: primitives that do not change with the theme ----
  const global: DtcgGroup = {};
  const rampSets: [string, Record<string, string>][] = [['BrandRamp', t.color?.brandRamp ?? {}]];
  for (const [name, ramp] of Object.entries<any>(t.color?.otherBrandRamps ?? {})) {
    rampSets.push([`BrandRamp${name.replace(/^brand/, '')}`, ramp]);
  }
  for (const [groupName, ramp] of rampSets) {
    const g: DtcgGroup = {};
    for (const [stop, hex] of Object.entries<any>(ramp)) {
  ...
  const dimensionGroup = (source: Record<string, string> | undefined, label: string): DtcgGroup => {
    const g: DtcgGroup = {};
  ...
  const families: DtcgGroup = {};
  for (const [key, stack] of Object.entries<any>(t.typography?.fontFamilies ?? {})) {
  ...
  const weights: DtcgGroup = {};
  for (const [name, w] of Object.entries<any>(t.typography?.fontWeights ?? {})) {
    weights[name] = token('fontWeight', Number(w), `Fluent 2 font weight ${name} = ${w}.`, name);
  }
  global.FontWeight = weights;

  const durations: DtcgGroup = {};
  for (const [name, d] of Object.entries<any>(t.motion?.durations ?? {})) {
  ...
  const themeModes: [string, string, any][] = [
  ...
  for (const [mode, filename, semantic] of themeModes) {
    ...
    const colors: DtcgGroup = {};
    for (const [name, value] of Object.entries<any>(semantic)) {
    ...
    const document: DtcgGroup = { Color: colors };
    files.push({ filename, collection: 'Fluent Theme', mode, document, tokenCount: count(document) });
  }

  const manifest: DtcgBundle['manifest'] = { name: 'Fluent 2', collections: {} };
  for (const f of files) {
    manifest.collections[f.collection] ??= { modes: {} };
    manifest.collections[f.collection].modes[f.mode] = [f.filename];
  }

  const notExpressible: DtcgBundle['notExpressible'] = [
    {
      category: 'shadow (elevation)',
      tokens: Object.keys(t.shadow ?? {}).length,
      reason:
        'Fluent shadows are multi-layer CSS box-shadow strings ("0 0 2px rgba(0,0,0,0.12), 0 8px 16px rgba(0,0,0,0.14)"). DTCG does define a composite "shadow" type, but the plugin\'s tokenTypeToFigmaType returns null for it, and Figma has no shadow VARIABLE at all — shadows are effect STYLES. Emitting them would produce "Failed to update a variable of type shadow". Recreate elevation as Figma effect styles by hand, or read the values from fluent_get_token.',
    },
    {
      category: 'motion curves (easing)',
      tokens: Object.keys(t.motion?.curves ?? {}).length,
      reason:
        'Fluent easing tokens are cubic-bezier() strings. DTCG defines "cubicBezier", but the plugin does not handle it and Figma has no curve variable type. Durations ARE exported (as $type duration); curves are not. Read them from fluent_get_token instead.',
    },
    {
      category: 'typography ramp (composite text styles)',
      tokens: Object.keys(t.typography?.ramp ?? {}).length,
      reason:
        'Entries such as body1 / caption1Strong bundle fontFamily + fontSize + lineHeight + fontWeight. DTCG defines a composite "typography" type; the plugin does not handle it, and Figma models these as Text STYLES, not variables. Every constituent primitive IS exported (FontFamily, FontSize, LineHeight, FontWeight), so the ramp can be rebuilt as Figma text styles that bind to those variables.',
    },
  ];
...
export const PLUGIN_CAPABILITY_STATEMENT =
  'We cannot run Figma plugins. This plugin cannot run, invoke, automate, or trigger a Figma community plugin: ' +
 ...
 */
function catalogLabel(v: boolean | null | undefined): string {
  if (v === true) return 'confirmed in the Figma MCP Catalog';
  if (v === false) return 'NOT in the Figma MCP Catalog — connection will fail';
...
export function registerFigma(server: McpServer): void {
  server.registerTool(
    ...
    async ({ section, host, maxChars, dtcgFile }) => {
      const data = load();
      if (!data) return textResult('Figma dataset not found at mcp/data/figma.json.');
      ...
      if (host) {
        ...
        return textResult(
          capped(
            ...
            maxChars,
            `Host "${hit.id}" carries more detail than the cap allows.`,
          ),
        );
      }
      ...
      switch (section) {
              ...
              JSON.stringify(
                {
                  ...
                  diagnose:
                    'Run the whoami tool first — it is rate-limit exempt and reports the authenticated email, every plan the user belongs to, and the seat type in each. That distinguishes "wrong account" from "out of quota".',
                },
                null,
                2,
              ),
              maxChars,
              'Request a narrower section.',
            ),
          );
        case 'hosts':
          return textResult(
            capped(
              ...
              maxChars,
              'Pass host:"<id>" to get one client instead of every client.',
            ),
          );
        case 'servers':
          return textResult(
            capped(JSON.stringify({ servers: data.servers, notes: data.meta }, null, 2), maxChars, 'Request a narrower section.'),
          );
        case 'kits':
          return textResult(
            capped(
              ...
              JSON.stringify(
                {
                  ...
                  seeAlso:
                    'fluent_figma_guidance { section: "tokens-export" } generates the DTCG file that the Variables Import plugin consumes — the one place this data set actually feeds a plugin.',
                },
                null,
                2,
              ),
              maxChars,
              'Ask for one plugin by name instead of the whole list.',
            ) +
              provenanceFooter(data.unverified, {
        ...
        case 'tokens-export': {
          ...
          if (!bundle) return textResult('Token dataset not found at mcp/data/fluent-tokens.json, so no DTCG export could be built.');
          ...
          if (dtcgFile) {
            ...
            const hit =
              bundle.files.find((f) => f.filename.toLowerCase() === key) ??
            ...
            if (!hit) {
              return textResult(
                `No DTCG file "${dtcgFile}". Available: manifest, ${bundle.files.map((f) => f.filename).join(', ')}.`,
              );
            }
            return textResult(
              capped(
                ...
                maxChars,
                `${hit.filename} holds ${hit.tokenCount} tokens. Raise maxChars, or run "node scripts/figma/dtcg-export.mjs" to write the real files to disk.`,
              ),
            );
          }
          ...
          return textResult(
            capped(
              JSON.stringify(
                {
                  what:
                    'Fluent 2 design tokens as Design Token Community Group (DTCG) JSON, ready for Microsoft\'s "Variables Import" Figma plugin. This is the code -> Figma direction; the Figma MCP server only reads the other way.',
                  weCannotRunPlugins: PLUGIN_CAPABILITY_STATEMENT,
                  ...
                  sample: {
                    'fluent-global.tokens.json → BrandRamp.80': (bundle.files[0]?.document as any)?.BrandRamp?.['80'],
                    'fluent-global.tokens.json → Spacing.spacingHorizontalM': (bundle.files[0]?.document as any)?.Spacing?.spacingHorizontalM,
                    'fluent-theme-light.tokens.json → Color.colorBrandBackground': (bundle.files[1]?.document as any)?.Color?.colorBrandBackground,
                  },
                  codeSyntax:
              ...
              maxChars,
              'Pass dtcgFile to fetch one document.',
            ),
          );
        }
        case 'code-connect':
        ...
        default: {
          ...
          for (const [key, parts] of Object.entries(sectionSources)) {
            entries[key] = {
              chars: parts.reduce<number>((n, p) => n + sizeOf(p), 0),

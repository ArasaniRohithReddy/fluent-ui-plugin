...
import { getPointsOnCurvePath, type Curve, type CurvedHelixPath } from './geometry.js';
...
const pointsForKeyColor = (keyColor: string, range: number[], _centerPoint: number): number[] => {
  const hue = hexToHue(keyColor);
...
const getLogSpace = (min: number, max: number, n: number): number[] => {
...
export function curvePathFromPalette({ keyColor, darkCp, lightCp, hueTorsion }: Palette): CurvedHelixPath {
  const blackPosition: Vec3 = [0, 0, 0];
  const whitePosition: Vec3 = [100, 0, 0];
  const keyColorPosition = LCH_to_Lab(keyColor);
  const [l, a, b] = keyColorPosition;

  const darkControlPosition: Vec3 = [l * (1 - darkCp), a, b];
  const lightControlPosition: Vec3 = [l + (100 - l) * lightCp, a, b];

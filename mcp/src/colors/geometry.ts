...
//   packages/react-components/theme-designer/src/colors/geometry.ts
// which is itself ported from ThreeJS (MIT). The arc-length reparameterisation matters:
// points are sampled by *distance along the curve*, not by the Bezier parameter t, so a
// naive `getPointOnCurve(t)` port produces a visibly different ramp.

import type { Vec3 } from './csswg.js';
...
function distanceTo(v1: Vec3, v2: Vec3): number {
  return Math.sqrt(distanceToSquared(v1, v2));
}

function distanceToSquared(v1: Vec3, v2: Vec3): number {
  const dx = v1[0] - v2[0];
  const dy = v1[1] - v2[1];
  const dz = v1[2] - v2[2];
...
function getPointOnCurve(curve: Curve, t: number): Vec3 {
...
function getCurvePathLength(curvePath: CurvePath): number {
  const lengths = getCurvePathLengths(curvePath);
  return lengths[lengths.length - 1];
}

function getCurvePathLengths(curvePath: CurvePath): number[] {
...
function getCurveLength(curve: Curve): number {
  const lengths = getCurveLengths(curve);
  return lengths[lengths.length - 1];
}

function getCurveLengths(curve: Curve, divisions = curveResolution): number[] {

...
// Colour-space maths ported from microsoft/fluentui:
//   packages/react-components/theme-designer/src/colors/csswg.ts
// which is itself the CSSWG CSS Color 4 reference code (Lea Verou / Chris Lilley, MIT)
// plus LeaVerou/css.land's `snap_into_gamut`.
//
// Only the functions the brand ramp needs are ported. Every constant is byte-for-byte
// upstream's; do not "simplify" the matrices or the D50 white point — the Theme Designer
// works in D50-adapted CIE Lab, and a D65 Lab shortcut silently produces a different ramp.

export type Vec3 = [number, number, number];

function multiplyMatrix3(M: number[][], v: Vec3): Vec3 {
  return [
...
/** sRGB -> linear-light sRGB (extended transfer function: negatives reflect through the origin). */
...
/** Linear-light sRGB -> gamma-corrected sRGB. */
...
function is_LCH_inside_sRGB(l: number, c: number, h: number): boolean {
  ...
  return rgb.reduce((a: boolean, b: number) => a && b >= 0 - e && b <= 1 + e, true);
}

/**
 * Moves a Lab colour into the sRGB gamut by holding L and H steady and binary-searching
 * chroma down until the colour sits on the sRGB boundary.
 */
